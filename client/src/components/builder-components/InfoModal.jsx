...
function InfoModal({ post, modifyPost, setShowInfoModal }) {
  ...
  const handleTitleImageUpload = (e) => {
    ...
    axios
      .post(
        `/image_components?post_id=${post._id}&current_key=${post.display_image_key}`,
        form
      )
      .then((response) => {
        setIsLoading(false);
    ...
    <div className='modal-container'>
      <div className='modal-wrapper'>
        <div className='info-modal'>
          <p
            className='close expand-cursor reacting-link'
            onClick={() => {
              setShowInfoModal(false);
            }}
          >
                ...
                return (
                  <div key={option._id}>
                    <input
                      ...
                      id={option._id}
                      onClick={(e) => {
                        handleTagClick(option, e.target.checked);
                      }}
                      defaultChecked={
                        tags.findIndex((t) => t._id === option._id) > -1
                      }
                    />
          ...
          {!isLoading && (
            <div className='submit-button'>
              <p className='expand-cursor reacting-link' onClick={submit}>
                Submit
              </p>
            </div>
          )}
